...
The core of the bridge is the \texttt{\_on\_message} callback. Each time
a Unity MQTT publish arrives, the callback parses the JSON payload,
resolves the VSS path from the pre-built lookup table, and writes the
value to Kuksa via gRPC in one call:

\begin{lstlisting}[language=Python,
  caption={MQTT-to-Kuksa bridge callback from feeder.py.},
  label={code:feeder-msg}]
def _on_message(self, client, userdata, msg: mqtt.MQTTMessage):
    ...
    try:
        ...
        self._kuksa.connect()   # attempt to reconnect on next call
\end{lstlisting}

The reconnect-on-failure logic inside \texttt{set\_value} means the
feeder survives a brief Kuksa databroker restart without manual
intervention --- important when the container orchestrator restarts
...
The subscription setup and the safety check callbacks for overspeed are
shown below. The pattern is identical for the RPM redline check:

\begin{lstlisting}[language=Python,
  caption={Velocitas subscription setup and overspeed handler from app.py.},
  label={code:velocitas-sub}]
...
Two details are worth noting. First, the flag \texttt{\_overspeed\_active}
prevents a flood of repeated alert messages when the vehicle is
continuously above the threshold --- only the first crossing and the
recovery generate a message. Second, the alert payload is sent to
\texttt{leda/command/alert}, a topic that Unity already subscribes to via
the \texttt{leda/command/\#} wildcard in \texttt{LedaBroker.cs}, so the
Editor health window will surface it without any additional wiring.
